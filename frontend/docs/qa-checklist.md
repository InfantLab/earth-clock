...
### Earthquakes
- [x] Menu → Geology → Earthquakes toggle shows/hides the layer
- [x] Markers cluster along real seismic zones (Ring of Fire, Mid-Atlantic
      Ridge, Himalayan front) — not randomly scattered
- [x] Bigger markers for higher-magnitude events (compare a M6+ vs a M2 quake
      if both are present in the current week's feed)
- [x] Colour reads shallow = red, deep = blue (Pacific subduction zones like
      Indonesia/Japan should show more blue than continental rift quakes)
- [x] Events under ~24 h old visibly pulse; week-old events sit as a steady,
      fainter dot
- [x] Time-warp forward or backward more than 24 h from now (`window.__orreryTimeWarp
      = 3600*24*50` in console) — earthquakes hide, same as Fires/Hurricanes.
      Return to near-now — they reappear.
- [x] Data panel (View → Data) shows an `earthquakes` row: source, event
      count, "last 7 days"
...
### Tectonic plates
- [x] Menu → Geology → Plates toggle shows/hides the layer
- [x] Amber/terracotta lines trace real plate boundaries — visually distinct
      from the cool-white coastlines even with both on
- [x] Time-warp far from now — plates stay visible (static layer, no
      freshness gate, unlike earthquakes)
- [x] Flat map view: boundaries render correctly, no antimeridian seam
      artifacts (check near the Aleutians/Kamchatka, ~180° longitude)

### Volcanoes
- [x] Menu → Geology → Volcanoes toggle shows/hides the layer
- [x] Small triangular markers visible along volcanic arcs (Japan, Indonesia,
      Andes, Mediterranean) — zoom in if they're too small to distinguish
      from the surrounding terrain texture at default zoom
- [x] Dormant volcanoes render as a dim rock colour; check at least one known
...
- [x] Time-warp far from now — volcanoes stay visible (static, no freshness
      gate)
- [x] Data panel shows a `volcanoes` row: source, volcano count, bundled

### Active eruptions (FIRMS cross-reference)
- [x] With both Fires and Volcanoes toggled on, at least one hot volcano
      marker sits near (not necessarily exactly on top of, given the 20 km
      match radius) a cluster of orange fire dots
- [x] Toggle Fires off — erupting volcanoes keep their hot colour (the flag
      persists on the volcano layer independent of fire-dot visibility)
- [x] Wait for an hourly fire refresh (or reload the page) — erupting set
...
- [x] Geography row still shows only Coastlines + Night lights (Geology is a
      separate row, not merged into Geography)
- [x] Fires, Hurricanes, Lightning, Aurora all still behave exactly as before
      (freshness gating, mobile auto-collapse, flicker/decay animations)
- [x] Menu label column width still fits "Geography"/"Geology" without
      wrapping the " | " separator
- [x] Fresh `localStorage` (clear `orrery.menu.v1`): new Geology toggles
      default to on, matching DEFAULTS
- [x] Mobile (≤600 px): toggling any Geology layer auto-collapses the menu,
      same as every other row

### Browser matrix
- [x] Chrome / Edge (Chromium) — primary; already covered by the automated
      Playwright pass above, spot-check only
- [x] Firefox — new `ShaderMaterial`s in EarthquakeLayer/VolcanoLayer are the
      risk surface (custom GLSL point-sprite shaders); confirm markers render
      identically, no shader compile errors in console
- [x] Safari macOS — same shader risk as Firefox
- [x] Safari iOS / Chrome mobile — Geology row toggles are reachable and
