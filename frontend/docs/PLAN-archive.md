# Orrery — Plan Archive (historic + reference)

> **This document is an archive.** The live engineering tracker now lives at the top
> level: **[/PLAN.md](../../PLAN.md)**. This archive captures the full per-layer
> history — every "✅ Done" entry, every "🔄 In progress" note, every implementation
> detail captured during development. Keep this around as reference for *how* each
> piece works and the rationale behind it; do not update it. New work goes in the
> top-level plan.
>
> Archived from `orrery/PLAN.md` on 2026-05-26, just before the v0.1.x UX-polish round.

---

For the project vision, lineage, architecture, data sources, and astronomical formulae, see [README.md](../README.md).
...
### 🔄 Solar-eclipse visualisation tool  ← **headline feature**, v1 landed; lunar-model upgrade TODO
...
**v1 status (2026-05-16, v0.0.5)** ✅
- Path of totality + live umbra disc landed.
- Magnitude threshold relaxed 1.0 → 0.95 to absorb model-precision wobble.
- Schlyter lunar-position **epoch bug** found and fixed (was using J2000 instead of Schlyter's 1999-12-31 epoch — 1.5-day offset → 19.6° lunar-anomaly error).
- Main-perturbation terms (evection / variation / yearly / etc.) added on top of basic Keplerian. Brings the runtime lunar model to ~0.5° angular accuracy.
- **0.5° lunar error magnifies to ~30° on Earth's surface** at eclipse alignment — the geometry is brutally sensitive. So for catalogued headline events we now use NASA's published centerline directly (next bullet) rather than computing from the runtime lunar.

**v1 status — catalogued NASA paths** ✅
- [src/data/nasaEclipsePaths.ts](src/data/nasaEclipsePaths.ts) — hand-typed centerline waypoints from NASA's predictions for 2024-04-08, 2026-08-12, 2027-08-02. Linear interpolation at runtime via `interpolateUmbraPosition()`.
- main.ts dispatch: catalogued eclipses use NASA centerline (accurate, deterministic); non-catalogued events fall back to the Schlyter computation.
- Replacing approximations with denser NASA SVS data is a copy-paste job documented in the file's footer.

**v2 status — Eclipse panel** ✅ *(landed alongside Tools-menu retirement)*
- Dedicated [src/ui/EclipsePanel.ts](src/ui/EclipsePanel.ts), top-left under the Clock — small, time-anchored, lists the bundled catalogue with a one-click "jump" per event.
- Menu's "Eclipse" entry now toggles this panel; the underlying `EclipseLayer.mesh.visible` is driven by the same toggle so 3D rendering and panel visibility stay in sync.
- Tools menu retired in the same change: `Use test data`, `Find moon`, and `Jump to eclipse` moved from the bottom-right Debug panel into `window.__orrery` console helpers (`__orrery.useTestData()`, `__orrery.useLiveData()`, `__orrery.findMoon()`, `__orrery.jumpToEclipse(id?)`). Astro readouts dropped — the Data panel and the Clock already cover that ground.

**TODO — runtime lunar precision (Phase A polish)** ⬜
- Implement **Meeus simplified ELP-2000-82B** in `src/astro/lunar.ts`. ~30 main perturbation terms + a handful of nutation corrections → 0.1° angular accuracy, enough for shadow geometry on ANY date (not just catalogued).
- Once landed: non-catalogued eclipses render correctly without hand-typed centerline data; we keep `nasaEclipsePaths.ts` for the headline events because it's still more precise than even Meeus + ray-sphere.
- Estimated effort: half-day. Reference: Meeus, "Astronomical Algorithms" 2nd ed., chapter 47 ("Position of the Moon"). The Python `astral` library has a clean port if a reference implementation helps.
...
9. **Hide-in-map-mode list** — Atmosphere, Sky-stars toggles should be greyed in the menu when Map mode is on, since they're inherently 3D and don't have flat-map equivalents.
...
11a. **Eclipse on flat map** ⬜ — the 3D EclipseLayer currently renders only against the sphere. Port the umbra/penumbra discs + the path-of-totality polyline onto the equirectangular plane: lat/lon → (u, v) for the path; the live umbra centre is just a coloured disc at the sub-shadow point. Discs should fade in/out the same way the 3D meshes do when `setLiveShadow(null)` is fed. Same `setMapMode`-style hook as the rest of the FlatMap v2 ports.
...
| 2026-05-16 | **v0.0.5 huge bug-hunt session.** Two long-standing structural bugs found and fixed: (1) **Schlyter lunar epoch was J2000 instead of 1999-12-31** → 1.5-day offset → 19.6° mean-anomaly error → fundamentally broken moon position. (2) **VIIRS NOAA-20 clouds had been failing for the entire project life**: zoom 1 in the 250m TileMatrixSet has 144°-tall tiles, so the bottom row was 75% structural off-Earth black fill that triggered our no-data check. Switched to zoom 3 (10×5 matrix = 36°-tall tiles, fits the world exactly, 50 tiles instead of 6). Also: lunar perturbation terms added; computeShadow geometry hardened against near-miss precision; **NASA-catalogued eclipse paths** ([nasaEclipsePaths.ts](src/data/nasaEclipsePaths.ts)) replacing the runtime lunar-based path for headline events (2024 NA, 2026 Spain, 2027 long-Spain) since 0.5° lunar error magnifies to 30° on Earth at eclipse alignment. Eclipse path hidden when sim-time is far from window. **Time-warp ⏪ rewind** actually rewinds now (negative warp). Beams visual upgrade (tapered cylinders, sun corona, moon-phase disc on flat map). LocationPanel restructured for click-source consistency. Skybox loader landed (2K/8K Solar System Scope assets bundled). Sun rendered at true position with corona. | Browser-matrix QA · Meeus lunar-model upgrade (precision for non-catalogued events) · cutover |
