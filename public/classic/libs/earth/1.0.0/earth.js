...
(function () {
    ...
    function animate(globe, field, grids) {
        ...
        function draw(frameGlobe) {
            ...
            // Some D3 projections still return finite screen coords for clipped points, but only clip during streaming.
            ...
            if (hemisphereClip && currentProjection.rotate) {
                ...
                // In our globe models, user-facing center is [-rotate[0], -rotate[1]] (degrees).
                centerCoord = [-r[0], -r[1]];
            }
            ...
            function isFrontHemisphere(λ, φ) {
                ...
                // Use d3.geo.distance for robust hemisphere tests (avoids sign mistakes).
    ...
    // Moon texture state (module-level so drawMoonPhaseOverlay can access it)
    ...
    function loadMoonTexture(onReady) {
        ...
        img.onload = function () {
            ...
            try {
                ...
                if (onReady) onReady();
            } catch (e) { /* tainted canvas — proceed with procedural fallback */ }
        };
        img.onerror = function () { /* proceed with procedural fallback */ };
        img.src = "/images/moon.jpg";
    }

    /**
     * Smooth value noise for moon surface texture fallback (mare/highland variation).
    ...
    /**
     * Project the sub-lunar point onto the globe and draw the moon phase disc there,
     * clipping each pixel through the globe mask so the disc sits on the surface rather
    ...
    function drawMoonPhaseOverlay(globe, mask, moonData) {
        ...
        // ImageData covering just the disc bounding box
        ...
        for (var row = 0; row < dim; row++) {
            ...
            for (var col = 0; col < dim; col++) {
                ...
                // Clip to visible globe surface — naturally handles back-side and limb
                if (!mask.isVisible(canvasX, canvasY)) continue;

                var dist = Math.sqrt(dist2);
                var z    = Math.sqrt(r2 - dist2);

                // Illumination elevation above terminator
                ...
                // Sample lit colour from texture (bilinear) or procedural fallback
                var litR, litG, litB;
                if (_moonTexData) {
                    // Orthographic sphere → equirectangular UV
                    ...
                    // Procedural fallback (used until texture loads)
                ...
                // Dark side: near-black with faint earthshine toward centre
    ...
    function init() {
        ...
        function updateMoonPhase() {
            ...
            // Recalculate position at most once per minute (moon moves ~0.5°/min)
            var now = Date.now();
        ...
        function startMoonUpdates() {
            ...
            // Load texture then draw; falls back to procedural if load fails
        ...
        // Expose for Wallpaper Engine integration
